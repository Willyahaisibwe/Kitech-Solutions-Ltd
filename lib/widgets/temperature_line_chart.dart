  ...
  Widget build(BuildContext context) {
    return Consumer<HistoricalReadingViewModel>(
      builder: (context, viewModel, child) {
        ...
        final List<FlSpot> actualTempSpots = readings.asMap().entries.map((entry) {
          return FlSpot(entry.key.toDouble(), entry.value.temperature);
        }).toList();

        final List<FlSpot> desiredTempSpots = readings.asMap().entries.map((entry) {
          return FlSpot(entry.key.toDouble(), entry.value.desiredTemp);
        }).toList();

        // Calculate Y-axis scaling
        final double minTemp = readings.map((r) => r.temperature).reduce((a, b) => a < b ? a : b);
        final double maxTemp = readings.map((r) => r.temperature).reduce((a, b) => a > b ? a : b);
        final double overallMinY = [minTemp, ...readings.map((r) => r.desiredTemp)].reduce((a, b) => a < b ? a : b);
        final double overallMaxY = [maxTemp, ...readings.map((r) => r.desiredTemp)].reduce((a, b) => a > b ? a : b);
    ...
    return FlTitlesData(
      ...
      bottomTitles: AxisTitles(
        sideTitles: SideTitles(
          ...
          interval: (readings.length / 5).round().toDouble().clamp(1, readings.length.toDouble()),
      ...
      touchTooltipData: LineTouchTooltipData(
        getTooltipItems: (touchedSpots) {
          return touchedSpots.map((LineBarSpot touchedSpot) {
            ...
            final String label = touchedSpot.barIndex == 0 ? 'Actual' : 'Desired';
